...
class SticksGame:
    ...
    def try_removing_sticks(self):
        ...
        if self.sticks >= turn.removed_sticks:
            ...
            if not self.silent:
                print('Player {} removes {} sticks. {} sticks remain.'.format(
                    self.current_player.initial, turn.removed_sticks,
                    self.sticks))
            return
        else:
...
class Player:
    ...
    def fill_hats(self):
        if self.hats is None:
            self.hats = [{'in': [1, 2, 3], 'out': []} for _ in
                         range(self.sticks + 1)]
...
class HumanPlayer(Player):
    def choose(self):
        choice = input(
            "Player {}: {} sticks remain. How many sticks do you take (1-3)?".format(
                self.initial, self.sticks))
